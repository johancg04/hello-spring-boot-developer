...
public class GreetingsController {
    private static final Logger
            ...
            getLogger(GreetingsController.class);

    @GetMapping
    public ResponseEntity<GreetDeveloperResponse> greetDeveloper(
            @RequestParam(required = false) String firstName,
            @RequestParam(required = false) String lastName) {
        Developer developer = (Objects.isNull(firstName)) ||
                                (Objects.isNull(lastName)) ?
                null : Developer.builder().firstName(firstName)
                        .lastName(lastName).build();

        GreetDeveloperResponse response =
                GreetDeveloperAssembler.toResourceFromEntity(developer);
        LOGGER.info("Greeted developer: {}", response.fullName());
